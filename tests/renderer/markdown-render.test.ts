...
} from '../../src/renderer/platform/markdown';
...
describe('safe Markdown DOM rendering', () => {
  it('allows https images while keeping every other renderer source local', async () => {
    const harness = await createRendererHarness();
    try {
      const policy = harness.query<HTMLMetaElement>(
  ...
  it('renders a remote image inline without exposing the referrer', async () => {
    ...
    await renderer.renderInto(
      container,
      '![架构图](https://images.example.com/diagram.png?size=large)',
    );

    const image = container.querySelector('img');
    expect(image?.getAttribute('src')).toBe('https://images.example.com/diagram.png?size=large');
  ...
  it('drops an image whose URL uses an unsupported scheme', async () => {
    const renderer = createRenderer({});
    const container = document.createElement('div');

    await renderer.renderInto(container, '![payload](javascript:alert(1))');
