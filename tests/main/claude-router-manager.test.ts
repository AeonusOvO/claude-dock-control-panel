...
describe('Claude Code Router management', () => {
  ...
  it('starts both the CLI management service and port 3456 as part of saving the first provider', async () => {
    const access = {
    ...
    const internals = manager as unknown as {
      ...
      requireActiveService: () => Promise<typeof access>;
    ...
    const stoppedManager = new ClaudeRouterManager('C:\\claudedock-test');
    const stoppedInternals = stoppedManager as unknown as typeof internals;
    stoppedInternals.getActiveServiceAccess = vi.fn(async () => access);
    stoppedInternals.requireCliOwnedService = vi.fn(async () => undefined);
    stoppedInternals.rpcWithAccess = internals.rpcWithAccess;
    stoppedInternals.saveConfigWithoutProfileTakeover = internals.saveConfigWithoutProfileTakeover;
    stoppedInternals.getState = vi.fn(async () => ({
      ...runningState,
    ...
    try {
      await expect(stoppedManager.saveProvider(providerInput)).rejects.toThrow(
        'CCR 模型接口自动启动失败：模型网关仍未运行。',
      );
    } finally {
      nowSpy.mockRestore();
    }
