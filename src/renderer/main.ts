...
const createDownloadTaskCard = (task: DownloadTaskView, historical: boolean): HTMLElement => {
  ...
  if (!historical && task.canPause) {
    appendDownloadAction(actions, task, 'pause', '暂停');
  }
  if (!historical && task.canResume) {
    appendDownloadAction(actions, task, 'resume', '继续');
  }
  if (!historical && !['cancelled', 'completed', 'failed'].includes(task.state)) {
...
const applicationDownloadView = (): DownloadTaskView | undefined => {
  const state = applicationUpdaterState;
  if (!state || state.phase !== 'downloading') return undefined;
  const totalBytes = state.totalBytes ?? 0;
  const receivedBytes = state.downloadedBytes ?? 0;
  return {
    bytesPerSecond: state.bytesPerSecond ?? 0,
    canPause: false,
    canResume: false,
    elapsedMs: 0,
    id: 'application-update-download',
    label: `ClaudeDock ${state.latestVersion ?? ''} 应用更新`,
    percent: state.percent ?? -1,
    receivedBytes,
    remainingMs:
      state.bytesPerSecond && totalBytes > receivedBytes
        ? ((totalBytes - receivedBytes) / state.bytesPerSecond) * 1_000
        : -1,
    state: 'progressing',
    totalBytes,
  };
    ...
    items.push({
      ...
      run: async () => {
        await runRouterOperation(
          ...
          routerInstallSource.value === 'github' ? '正在下载并校验…' : '正在更新…',
          installRouterButton,
        );
      },
