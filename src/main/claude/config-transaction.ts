...
/**
 * Runs one prepare → commit → complete write against a project's Claude configuration under snapshot
 * rollback. Declared here so every IPC domain that writes project configuration injects the same
 * signature instead of restating it; the assembly owns the implementation because it closes over the
 * transaction coordinator and the workspace.
 */
