...
- 顶级活动栏新增“对话”：它不启动 PowerShell，也不复用项目级 Claude Code 配置。用户可以
  单独配置 Anthropic Messages 或 OpenAI Chat Completions 兼容接口、认证方式和任意合法模型
  ID；请求由主进程流式执行，支持停止生成和新建运行期对话，凭据只以 Windows 安全存储密文
  保存。
...
[roadmap.md](roadmap.md) 记录本轮三项需求的实施结论，以及聊天正文持久化、隐私与数据设置、
更多语言包等尚未纳入首期的后续方向。当前已上线能力以本 README 和代码为准。
...
- “对话”是独立于项目终端的通用模型聊天页，配置保存在
  ...
  兼容接口。首期消息只保留在本次应用运行期，不长期保存聊天正文。
...
1180px 全局设置两个分类、独立对话、终端聚焦态和重命名弹窗的本地视觉检查图到
`dist/visual-qa/`。
...
需要通用模型聊天时，点击活动栏“对话”，在左侧保存独立协议、接口、模型和凭据后即可发送。
这里的模型与项目“接入”可以完全不同；“停止”只中断当前生成，“新对话”只清空本次运行期的
独立消息，不影响任何项目或 Claude Code session。全局主题、开机启动和版本信息位于左下角
“设置 → 总设置”。
...
- 独立对话配置保存在 `userData/claude/chat-profile.json`，凭据同样只存
  ...
  使用 HTTPS（本机回环可使用 HTTP），首期不会把独立聊天正文写入磁盘。
