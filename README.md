...
- 统一全部滚动区域的样式；左侧控制栏与右侧工作台都可拖动调整宽度并记住设置。再次点击当前
  活动栏按钮会收起左侧控制栏，只保留三个常驻入口，再次点击任一入口即可展开对应页。
...
- “接入”页在高级设置与模型配置之间保留每个项目最近 20 条历史配置。保存时如果所填内容与
...
覆盖项目、接入、插件三个分组、工作台三页、收起的左栏和高级设置模态窗口；若交互组件被
透明层挡住、彼此重叠或关键容器横向溢出则失败。
`npm run test:visual` 会生成 820px 插件页、1180px 单组展开接入向导、1180px 历史配置组件、
1180px 高级设置窗口、终端聚焦态和重命名弹窗的本地视觉检查图到 `dist/visual-qa/`。
...
5. 在第一步选择密钥所属服务商。目录会自动填好真实 Anthropic 兼容基址、认证方式、主模型和
   ...
   后续表单随之收起。
6. 点击服务商区域下方“打开高级设置”，在模态窗口中调整认证来源，或使用自动发现、手动配置、
   ...
   - cURL 中的 Bearer 上游密钥交给 Router；如果 Router 自己启用了访问保护，
     ...
     高级设置下方的“选择历史配置”会直接列出曾填写的接口/网关、主模型和快速模型；需要复用时
