...
- 终端底栏的“连接待测试”改成真按钮：点击立刻切到“接入”页并直接开始真实连接测试，按钮进入
  转圈的等待态并禁止重复点击，测试结束后自动恢复成连接正常/需确认/异常。
...
- 底栏新增“模式”，列出 Claude Code 的全部权限模式：手动确认、自动接受编辑、计划模式、
  ...
  模式徽标与底栏显示实时同步。
- 点击模式列表时逐次发送 `Shift+Tab` 并回读终端徽标确认，命中目标才停；`auto` 这类是否
  存在取决于账号与模型的模式不会切歪，切不到时直接提示“该模式在当前会话不可用”。
...
- 底栏的连接、模型和模式三处都可点击：连接直接开始真实测试并显示等待态；模型与模式弹出
  浮层列表，宽窗口显示、900px 以下随底栏一起收起。
...
- 权限模式的真值来自 Claude Code 自己重绘的模式徽标，`statusLine` 数据里没有这一项。
  底栏切换按一次 `Shift+Tab` 就回读一次徽标，最多 8 轮；同一会话的并发点击会被拒绝而不是
  叠加按键。
...
12. 提示词写在终端输出区下方的输入框里，`Enter` 发送、`Shift+Enter` 换行、`↑/↓` 翻历史；
    ...
    二次确认。
13. 底栏左侧的连接按钮可以随时重跑真实连接测试：点击后立即切到“接入”页并开始测试，按钮进入
    等待态且不可重复点击。底栏中间的“模型”弹出可切换的模型列表，同端点直接切、跨端点确认后
    压缩并重启；右边的“模式”列出全部权限模式，也可以在终端或输入框里按 `Shift+Tab` 切换，
    PowerShell 里的徽标与底栏显示保持一致。想使用“完全允许”时，需要工作台的
