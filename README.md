...
当前版本 **5.0.0-rc.17**，仅发布 Windows x64。
...
1. 新配置会自动打开四步入门向导；先选择 Claude Code、ChatGPT / Codex 或 API 服务商路径。
2. ClaudeDock 检测本机环境，再通过 Windows 文件夹选择器打开第一个工作区。
3. 进入工作区后，ClaudeDock 沿既有自动流程补齐组件、选择路由、发现模型、真实测试并保存。
4. 之后可从「工作区」继续会话，从「接入」管理服务商，从「扩展」切换插件与 MCP；设置中可重新打开入门向导。
