...
- 服务商类别都可点击标题展开或收起，并带箭头与过渡效果；服务商卡片根据侧栏实际宽度自动
  ...
  保留服务商选择和高级设置入口。
- “高级设置”改为主题自适应的模态窗口，集中承载网关、cURL、Router、诊断和历史工具，并提供
...
`npm run test:visual` 会生成 820px 插件页、1180px 接入向导、1180px 高级设置窗口和重命名
弹窗的本地视觉检查图到 `dist/visual-qa/`。
...
7. 点击服务商区域下方“打开高级设置”，在模态窗口中使用自动发现、手动配置、Router 管理、
   cURL 识别、转换器说明、历史恢复与术语解释；完成后点“完成”，需要放弃未保存输入时点
