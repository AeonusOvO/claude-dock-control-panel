...
`npm run test:visual` 会生成 820px 插件页、1180px 接入向导、1180px 历史配置组件、
1180px 高级设置窗口和重命名弹窗的本地视觉检查图到 `dist/visual-qa/`。
...
5. 在第一步选择密钥所属服务商。目录会自动填好真实 Anthropic 兼容基址、认证方式、主模型和
   ...
   展开/收起；再次点击当前服务商会回到未选择状态，后续表单随之收起。
6. 点击服务商区域下方“打开高级设置”，在模态窗口中使用自动发现、手动配置、Router 管理、
   cURL 识别、转换器说明与术语解释；完成后点“完成”，需要放弃未保存输入时点
