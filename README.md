...
- 底栏新增“模式”，列出 Claude Code 的全部权限模式：手动确认、自动接受编辑、计划模式、
  ...
  模式徽标与底栏显示实时同步。
- 点击模式列表时逐次发送 `Shift+Tab`，等 xterm 应用完 Claude Code 的屏幕差量后回读完整徽标，
  命中目标才停；`auto` 这类是否存在取决于账号与模型的模式不会切歪，切不到时直接提示
  “该模式在当前会话不可用”。
...
- 权限模式的真值来自 Claude Code 自己重绘的模式徽标，`statusLine` 数据里没有这一项。
  Claude Code 会只输出变化的终端单元格，因此 ClaudeDock 等 xterm 合成出完整可见屏幕后再
  回报徽标；底栏切换按一次 `Shift+Tab` 就核对一次，最多 8 轮，同一会话的并发点击会被拒绝。
