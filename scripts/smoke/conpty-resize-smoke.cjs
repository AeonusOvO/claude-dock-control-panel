...
const installLifecycleRecorder = (window) =>
  window.webContents.executeJavaScript(
    `(() => {
      const requiredMethods = ['onTerminalData', 'onTerminalSize', 'onWorkspaceState'];
      const missingMethods = requiredMethods.filter(
        (method) => typeof window.controlPanel?.[method] !== 'function',
      );
      if (missingMethods.length > 0) {
        return {
          available: Object.keys(window.controlPanel ?? {}).sort(),
          missingMethods,
          ok: false,
        };
      }
...
const launchConptyHarness = async () => {
  await app.whenReady();
  await waitFor(
...
const assertInitialWorkspace = async (window) => {
  const initialWorkspaceResult = await window.webContents.executeJavaScript(
    `(async () => {
      try {
        return { ok: true, value: await window.controlPanel.getWorkspace() };
      } catch (error) {
        return {
  ...
  if (!initialWorkspaceResult.ok) {
    throw new Error(`读取初始工作区失败：${initialWorkspaceResult.error}`);
  }
  const initialWorkspace = initialWorkspaceResult.value;
...
const runResizeScenario = async (window) => {
  ...
  return { outputPath, resizeSequence, state };
};

const runLifecycleCycles = async (window) => {
...
const runSentinelShutdown = async (window, finalStatus) => {
  ...
  return { lifecycleEvents, sentinel };
};
...
const run = async () => {
  ...
  const ordering = assertLifecycleOrdering(
    lifecycleEvents,
    lifecycle.finalStatus.id,
    lifecycle.finalStatus.ptyGeneration,
    sentinel,
  );
