/* eslint-disable max-lines-per-function -- 单进程线性视觉场景脚本：capture 帧采样闭包与场景序列按时序耦合，拆分会破坏采样时序的可读性。 */
...
const loadThemeDefinitions = () => {
  const source = readFileSync(path.join(root, 'src', 'shared', 'ui', 'terminal-themes.ts'), 'utf8');
...
// A wall-clock wait alone cannot prove a transition reached its resting value: a hidden window's
// animation clock only advances when the compositor actually renders, and capturePage can hand
// back a cached frame under load, leaving the clock parked mid-transition (the 120ms send-button
// background once surfaced exactly this way as "the resting stop button still paints an outer
// ring"). Probe the transition-sensitive computed styles the later assertions read, force a
// fresh frame between probes, and only proceed once two consecutive probes agree. Paused-motion
// captures settle immediately because their held timelines stop changing.
const styleProbeScript = `
  (() => {
    ...
    return JSON.stringify([
      ...
      launch ? getComputedStyle(launch).backgroundColor + ':' + getComputedStyle(launch).opacity : null,
    ]);
  })()
`;

const settleCompositorStyles = async (window) => {
  const deadline = Date.now() + 1200;
  let previous = await window.webContents.executeJavaScript(styleProbeScript);
  for (;;) {
    await new Promise((resolve) => setTimeout(resolve, 90));
    await window.capturePage();
    const current = await window.webContents.executeJavaScript(styleProbeScript);
...
app
  ...
  .then(async () => {
    ...
    const capture = async (feature, file, metadata = {}) => {
      ...
      // Cached frames can still leave the animation clock mid-transition under load; hold the
      // probe stable across two forced frames before the authoritative capture and DOM sampling.
      await settleCompositorStyles(window);
      window.webContents.invalidate();
