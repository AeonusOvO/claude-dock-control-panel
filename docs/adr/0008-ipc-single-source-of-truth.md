...
演进过程：通道常量先从两侧字面量提取到 `src/shared/ipc/channels.ts`，载荷校验集中到 `src/shared/ipc/schema.ts`，preload 桥按域拆成 19 个文件，main 侧 handler 改为贡献点注册（见 [ADR-0010](0010-runtime-registry-and-contributions.md)）。本 ADR 记录收敛后的终局形态。
...
| 组成部分     | 位置                         | 规模（实测）                                                             |
| ------------ | ---------------------------- | ------------------------------------------------------------------------ |
| 通道常量     | `src/shared/ipc/channels.ts` | 188 个常量，分三组：请求响应 159、单向命令 7、事件推送 22                |
| 载荷校验     | `src/shared/ipc/schema.ts`   | 31 个导出 zod schema，其中 20 个经 `guarded<T>()` 包装为类型安全校验器   |
| preload 桥   | `src/preload/bridges/`       | 19 个按域拆分的桥文件，`index.ts` 展开组装并 `satisfies ControlPanelApi` |
| main handler | `src/main/ipc/`              | 23 个 IPC 域贡献，经 `MAIN_IPC_CONTRIBUTIONS` 数组注册                   |

通道常量按消息方向分三组，每组同时导出字符串字面量类型与冻结数组：

| 分组              | 通道数 | 消费方式                                                    |
| ----------------- | ------ | ----------------------------------------------------------- |
| `requestChannels` | 159    | preload `invoke` ↔ main `ipcMain.handle`                    |
| `sendChannels`    | 7      | preload `send` ↔ main `ipcMain.on`（高频写入，见 ADR-0002） |
| `eventChannels`   | 22     | main `webContents.send` ↔ preload `on`                      |
...
- **字面量清零判据**（机检）：188 个通道字符串值在 `channels.ts` 之外的 `src/` 全部 `.ts` 文件中出现次数为 0。实测脚本从 `channels.ts` 提取全部通道值后对 `src/` 其余文件做子串扫描，违规 0 处。
...
- 代价：188 个常量集中在一个 296 行文件里，按域注释分段；新增域时按字母序插入对应分组。
