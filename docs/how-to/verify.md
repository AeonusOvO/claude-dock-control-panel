...
控件，真实 CLI 拒绝在 IPC 边界检查；其余视觉交互仍要求实际窗口命中，不能用脚本穿过隐藏控件。
侧栏引擎选择验证“下一个对话”的偏好持久化，同时核对当前原生会话的 ID、引擎、正文和输入框保持不变。

国内订阅回归覆盖设备码、PKCE/state、真实回环回调、套餐确认、固定端口、独立账号绑定、共享刷新、
...
- 「检查所有更新」读取打包进应用的当前 COS 通道；检查本身不触发下载。只有一次显式“下载并更新”
  开始传输，下载和 SHA-512 校验完成后必须自动静默安装、重启，并显示新版本，不再要求第二次点击。
