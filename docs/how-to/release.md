...
正式发布必须让目标通道用户能够从软件右上角“检查更新”发现新版本，并在一次显式“下载并更新”后
完成校验、静默安装、自动重启和版本确认。
...
4. rc.15 需要完成一次手动安装后读取 COS `rc.yml` 的引导证明；后续完整生产更新证明必须覆盖
   检查不下载、一次显式“下载并更新”、下载校验、无需第二次应用内确认的静默安装、自动重启和版本
   确认。在完整序列真实通过前，发布记录只能标为待验证。
