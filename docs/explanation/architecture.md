...
**请求响应**（159 个频道）—— 渲染端 `invoke`，主进程 `handle`，返回结构化结果。全部用户操作走这条路。
...
**事件推送**（22 个频道）—— 主进程状态变化后广播，渲染端订阅并重渲染对应界面。
...
`src/renderer/main.ts` 从 15,886 行的单一作用域（HEAD `6ca456e` 基线实测；ADR-0006 时点记为 15,181）收敛为 33 行入口：14 个特性收进 `features/`（194 个文件），跨特性外壳收进 `shell/`，与特性无关能力收进 `platform/`，装配分为 `bootstrap.ts`（DOM 环境与外壳）与 `feature-registration.ts`（特性注册与解析）。
...
| 行为测试           | Vitest + 三个 harness            | 渲染端模块加载驱动、主进程服务、IPC 往返（1,096 例） |
