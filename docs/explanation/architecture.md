...
| 国内订阅授权与转发           | `SubscriptionService` / `SubscriptionRelay` | `subscription:state` / `subscription:changed`，只投影安全状态 |
...
启动自动接入也是 main 应用级事务：它使用不可见、不启动 PTY 的 workspace owner 进入既有可回滚配置事务，并把步骤、脱敏后的官方订阅账户、可取消时间与硬截止时间作为单调快照推送。页面切换、renderer reload 与多次进入“接入”只重放同一份主进程状态，不重建工作。取消和超时先中止精确 session operation，直到旧回调退栈、临时 owner 关闭后才解锁 UI；这项 blocking busy lease 也会进入退出确认快照。关闭“启动时自动接入模型”时，main 清空全局下个对话接入，renderer 不自动打开或启动历史对话，冷启动保持未接入。
...
三条独立通路共 219 个频道，形态与频率不同：

- **请求响应**（187 个）—— renderer `invoke`，main `handle`，返回结构化结果。
- **单向发送**（7 个）—— 高频或握手型 renderer → main 消息使用 `send/on`；包括 generation-fenced `terminal:write`，避免每次按键产生 Promise 往返。
- **事件推送**（25 个）—— main 状态变化后广播，renderer 订阅并重渲染对应界面。

`ControlPanelApi` 同样有 219 个成员，但分区不同：187 个请求方法、25 个事件订阅、6 个直接 send 方法和 1 个非 IPC `webUtils` 方法。第 7 个 send 频道由 `onAppQuitRequested` 的应答路径内部发出。完整映射见 [ipc-contract.md](../reference/ipc-contract.md)。

国内订阅授权由 main 独占，application 网络处理登录和刷新，conversation 网络处理实际上游请求。
内置回环服务一直持有已绑定 socket，每次授权形成独立账号 slot；旧会话不跟随新账号切换。
全局接入租约覆盖浏览器等待、真实测试和保存，ChatGPT 全局授权与其他写入共享同一边界。
每个 slot 的刷新只运行一次，取消单条推理不撤销其他请求依赖的轮换；退出撤销服务生命周期并拒绝迟到写入。
详细安全边界与未验证范围见[订阅账号接入](../reference/subscription-access.md)。
