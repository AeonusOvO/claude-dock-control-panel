...
三条独立通路共 212 个频道，形态与频率不同：

- **请求响应**（182 个）—— renderer `invoke`，main `handle`，返回结构化结果。
...
`ControlPanelApi` 同样有 212 个成员，但分区不同：182 个请求方法、23 个事件订阅、6 个直接 send 方法和 1 个非 IPC `webUtils` 方法。第 7 个 send 频道由 `onAppQuitRequested` 的应答路径内部发出。完整映射见 [ipc-contract.md](../reference/ipc-contract.md)。
