...
当前设计版本：**5.0.0-rc.17**（2026-08-24）。旧版设计计划、路线图、缺陷清单与分阶段修复提示词
统一归档在 [`docs/archive/`](../archive/)；它们仅用于历史追溯，不得覆盖本文的当前设计契约。

## 工作区优先导航与启动引导（2026-08-24）
...
- 新用户启动引导固定为“选择路径 → 自动准备 → 打开项目 → 准备完成”四步。路径包括 Claude Code、
  ChatGPT / Codex 和 API 服务商；检测结果必须区分“已就绪 / 待安装 / 项目后检测 / 自动”，未知状态
  不得伪装为成功。模型标识、路由内核和可实时发现的信息不要求普通用户手填。
...
- 环境准备卡同时显示 ClaudeDock/Claude Code 已安装版本、可检测到的最新版本和检查时间。
  ...
  自动择优，文案明确“不创建重复安装”。
- ClaudeDock 行必须保留完整预发布版本，例如 `v5.0.0-rc.16 → 5.0.0-rc.17`；可用、下载中和已下载
