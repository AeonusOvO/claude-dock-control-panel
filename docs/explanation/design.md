...
当前设计版本：**5.0.0-rc.29**（2026-08-26）。旧版设计计划、路线图、缺陷清单与分阶段修复提示词
...
- 项目默认进入真实 PowerShell/ConPTY 终端；项目标题旁的 `+` 是唯一新建对话入口，每次点击都立即在
  左栏插入“正在新建”行、在终端中央显示准备遮罩，并在后台创建独立 session。继续、选择历史和历史记录点击仍走
...
- “自动加载上次对话”和“自动连接最近一次选择的平台和模型”是两个独立、默认开启的开关。前者打开
  最后使用项目中最近的对话；后者在窗口出现前真实验证并恢复该对话绑定的平台、账户/API、接口、详细模型和简单模型，
  再把结果保存为应用级“下个对话接入”。两者
  都开时必须先启动候选模型所需的后台路由、完成真实测试与可回滚事务，再启动 CLI；保存配置相同不
  代表服务已经连接。只开模型时使用启动阶段的短生命周期事务恢复接入，不创建可见对话；只开对话时
  沿用当前接入恢复内容。关闭任一项不删除项目、对话或模型绑定。
...
- 勾选“自动连接最近一次选择的平台和模型”后，冷启动恢复期间整个接入画布置灰、轻微模糊并设为 inert，中央固定显示“正在接入模型”、真实阶段和倒计时。离开后再返回只恢复同一主进程事务的快照，不闪现“暂不可用”，不重复开始接入。默认 2 分钟后出现“取消接入”，默认 5 分钟由主进程强制中止；两个整分钟阈值在设置中可改，且硬截止必须晚于可取消时间。取消、超时或失败都要等待可回滚事务真正退栈才恢复原界面，不得提前显示成功或提前解锁。
