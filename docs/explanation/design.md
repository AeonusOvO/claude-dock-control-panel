...
当前设计版本：**5.0.0-rc.23**（2026-08-25）。旧版设计计划、路线图、缺陷清单与分阶段修复提示词
...
- ChatGPT 授权等待是可打断阶段：“上一步”先取消主进程拥有的 OAuth 子进程，收到完成结果后再返回。
  Proxy API 安装/配置、模型发现、真实测试、项目保存和修复属于不可打断阶段，此时“上一步”禁用并用
  状态文字说明原因。可打断性来自类型化 progress 事件和主进程二次门禁，不从按钮文字推断；其他项目
  的忙态不能取消当前项目以外的事务。
- 四入口卡片同时显示名称和一句用途；当前选择使用强调色描边与勾选，已保存项另显示“当前配置”文字。
...
- “自动加载上次对话”和“自动连接最近一次选择的平台和模型”是两个独立、默认开启的开关。前者打开
  最后使用项目中最近的对话；后者接回该对话绑定的平台、账户/API、接口、详细模型和简单模型。两者
...
- 接入页标题下固定显示脱敏的“当前接入”：API / 中转站优先显示用户保存的名称，缺失时显示去掉
