...
当前设计版本：**5.0.0-rc.18**（2026-08-25）。旧版设计计划、路线图、缺陷清单与分阶段修复提示词
...
- 动效遵循“来源可追踪、返回路径对称”的空间规则：引导从触发控件的 `transform-origin` 展开并沿同一
  ...
  运动。顶层接入/扩展切换只做短促淡化和容器变化，避免整屏横移造成错误层级。
...
- 环境准备卡同时显示 ClaudeDock/Claude Code 已安装版本、可检测到的最新版本和检查时间。
  ...
  自动择优，文案明确“不创建重复安装”。
- ClaudeDock 行必须保留完整预发布版本，例如 `v5.0.0-rc.17 → 5.0.0-rc.18`；可用、下载中和已下载
