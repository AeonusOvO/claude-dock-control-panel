...
- “自动加载上次对话”和“自动加载模型”是两个独立、默认开启的开关。前者打开最后使用项目中最近的
  对话；后者接回该对话绑定的平台、账户/API、接口、详细模型和简单模型。两者都开时必须先完成原模型
  的真实测试与可回滚事务再启动 CLI；只开模型时使用启动阶段的短生命周期事务恢复接入，不创建可见
  对话；只开对话时沿用当前接入恢复内容。关闭任一项不删除项目、对话或模型绑定。
