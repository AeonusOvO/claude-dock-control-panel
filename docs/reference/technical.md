...
当前架构版本：5.0.0-rc.37（2026-08-28）。版本化工作区启动引导以 main 进程持久化状态、
...
国内订阅的网络预检解析固定回环入口对应的实际平台上游，不将 `127.0.0.1` 当作公共域名的 DNS
改写失败；实际转发仍逐请求执行网络守卫。安全账户标识随已提交的账号绑定加密保存，经只读投影返回接入摘要。
renderer 的模型选择器仅在选择、当前接入或忙态发生变化时重建，终端状态轮询不替换展开的控件。
已验证且已保存的接入统一显示“模型已连接”，同步返回模型选择；失败、取消和迟到回调不触发成功提示。
...
rc.40 的 ChatGPT 额度通过 `managed-chatgpt-quota.ts` 直接关联托管网关账户；
`managed-chatgpt-quota-auth.ts` 仅异步、有界读取拥有的授权文件，不依赖另一套 Codex CLI 登录。
主进程在请求前后以账户/工作区身份与接入代次隔离结果，使用应用网络适配器执行可取消的只读请求。
接口兼容边界、失败提示与保密约束见上面的模型额度文档。
