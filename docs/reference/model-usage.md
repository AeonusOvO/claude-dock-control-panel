...
关闭按钮或卡片按钮可关闭；主窗口隐藏到托盘不关闭悬浮球，退出应用会关闭。新安装或迁移安装默认隐藏，
之后按用户最后一次明确选择在下次启动恢复；退出、托盘和窗口销毁不算明确隐藏。此显示偏好独立于额度、账户、
统计快照与位置，不会因额度刷新、模型/路由变化或普通快照发布自动弹出。两处显示同一份全局已接入模型的数据，
不随当前查看的旧对话改变；不同账户不混合统计。
...
托管网关生命周期开始时会使进行中的额度读取失效；若读取正在进行，生命周期全部结束后仅合并补发一次受账户代次约束的查询。
明确的账户/授权生命周期即使当前没有读取也会安排一次补查；普通启动、停止或配置生命周期不会强制刷新最近的缓存，仍遵守 60 秒轮询节流。
上述补查不启动网关、不运行预检、不登录或重新授权。生命周期期间已有窗口标为旧数据；同账户补查失败仍保留旧窗口，
确认新账户或授权不确定时才清除。两个窗口不各自轮询，打开悬浮球、切换主题不会增加查询。ChatGPT 查询全程限时 8 秒，响应上限 64 KiB；
