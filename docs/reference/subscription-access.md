...
全局接入、会话启动和历史恢复的预检，将本应用国内订阅的固定回环入口映射到该 provider 的真实上游；
`127.0.0.1` 不参与公共域名 DNS 判定，也不改写用户或系统 DNS。自定义接口不获得该映射。
本机转发仍校验 slot、端口与客户端口令，每条上游请求继续经过精确目标守卫。
...
GLM 平台令牌、授权 URL 与本机口令不经订阅 IPC 返回。成功配置视图返回已脱敏的 ClaudeConfigView，
以及可选的账户展示标识；不把授权成功阶段消息当作已保存配置。
最多保存 128 个独立 slot；不自动删除旧 slot，以免使已有对话改绑或失效。撤销账号授权或专用密钥请在官网操作。

## 账户显示

接入摘要分别标注“订阅 / API”。Claude 使用既有安全 CLI 账户投影，ChatGPT 使用托管网关验证后的邮箱；
国内账号只提取授权响应的 email、显示名或账户 ID 白名单，并随该 slot 的凭据一起加密保存。读取只匹配
已提交的 provider、固定端口与 slot，不启动 listener、不请求官网、不展示候选账号。刷新保留原绑定的账户。

JWT 只解析展示字段，不作为登录成功证据。拒绝控制字符、过长文本、令牌形态或与已知凭据相同的值；
UI 使用 textContent，不展示原始响应。无字段的旧账号绑定或手动套餐密钥显示“账户信息暂不可用”，不猜测
Windows 用户名、不截取密钥作为账户。重新登录后若官方返回账户字段则保存；不保证所有厂商都会返回。
...
账户回归覆盖显示字段净化、原账号刷新绑定、候选隔离、重启只读恢复和跨账号切换；网络回归覆盖全部五个
国内账号入口的实际平台预检目标，以及自定义配置仍使用自身目标。
