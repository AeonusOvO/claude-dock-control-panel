...
| `src/main/main.ts:5796-5806`                               | `ApplicationUpdaterService` 装配；`enabled` 仅 `allowApplicationUpdates && isPackaged && win32`                                                      | **执行阻断**：开发版、非 Windows 或关闭自动更新时服务处于 `disabled`，不发起任何更新请求                                                                                                  |
...
| `src/main/application-updater.ts:52-253`                   | `autoDownload = false`、`autoInstallOnAppQuit = false`、`allowPrerelease = false`、`allowDowngrade = false`、`checkAndDownload`、`installDownloaded` | **执行阻断**：禁止预发行版和降级；`latest.yml` 的 SHA-512 摘要不符或下载失败进入 `error`；未达到 downloaded 状态不能安装                                                                  |
...
## 5. 包含提示词、system prompt 或 subagent 行为说明的文档
...
测试失败会使对应测试命令非零；测试文件本身不是生产运行时门禁。
...
### 7.4 事务、Router、MCP 和 generation
...
| `src/main/software-updates.ts`                       | 展示软件更新可用性的版本比较        | 不是应用更新器的信任校验                    |
...
1. **客户端 updater 与配置事务的“回滚”语义不同**
   - 客户端 updater：禁止 downgrade，候选版本不得低于当前版本。
