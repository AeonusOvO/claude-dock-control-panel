...
- 左侧控制栏默认 320px (`--rail-w`)，通过 5px 分隔条在 270–560px 之间调整并持久化；
  ...
  时再次清理残留状态，不能让一个中断的拖拽继续拦截整个窗口的点击。
...
- 服务商按“官方接入 / 国内服务 / 海外与聚合服务 / 本地服务 / 高级方式”分组为双列卡片，
  窄侧栏降为单列。卡片同时显示名称和一句用途；当前选择使用强调色描边，已保存项另显示
  “当前配置”文字徽标，不能只用颜色表达。
- 选中服务商后才显示第二、三步。固定基址与认证方式由目录收敛；自定义、Router、cURL 才
  暴露对应高级字段。服务商控制台、官方文档和密钥不可混用等风险提示紧邻选择结果。
- 页面底部的“高级设置与诊断工具”默认折叠，完整容纳自动发现、手动配置、Router 管理、
  cURL 识别、转换器说明、接入历史和术语解释；选择 Router 或 cURL 时把对应完整工具临时
  提升到第二步，不复制功能或维护两套 DOM。
