...
当前架构版本：4.6.1（2026-08-07）。4.6.1 只新增 `当前版本需改进的bug.md`，记录终端 resize/主题、
会话恢复与唯一性、模型能力、任务和更新体验的审计结论、方案与待确认项；在用户确认前不改变 UI、
IPC、PTY、会话持久化或运行时行为。4.6.0 在原有按接入/模型隔离的服务速度 profile 和会话
...
2026-08-07 重新执行 `npm audit --omit=dev` 和完整 `npm audit` 时，当前锁文件报告 2 个条目：
`js-yaml@4.3.0` 的高危公告，以及 `mermaid@11.16.0` 聚合出的中危公告。`npm run check:licenses`
仍通过（186 个包、98 份不同许可文本）。4.6.1 在完成依赖升级、回归与重新审计前只能生成本地开发
验证包，不能晋升为正式稳定 Release；不得仅为清除报告而绕过 audit 或跳过功能/供应链回归。
