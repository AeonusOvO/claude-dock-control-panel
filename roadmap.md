...
本文件记录已经确认、尚未实现的产品任务。它不是当前功能说明；已上线行为仍以
...
| ---- | ------------------ | -------------------- | ------------ |
| 1    | 历史对话删除       | 已记录，待实现       | 1            |
| 2    | 独立“对话”选项卡   | 已记录，待设计与实现 | 3            |
| 3    | 全局设置入口与分类 | 已记录，待设计与实现 | 2            |

建议先补齐历史对话删除，再搭建全局设置外壳，最后实现独立对话。任务二的独立模型配置可以复用
任务三形成的设置导航和安全存储边界，但不得与 Claude Code 的项目级接入配置强绑定。
...
- 左侧历史对话当前支持左键恢复、右键重命名，没有删除菜单或可见删除按钮。
- preload 和主进程已经存在 `deleteClaudeSession` / `claude:delete-session` 通道，但 renderer
  没有调用它。
- 当前 `ClaudeSessionManager.deleteSession()` 会直接 `unlinkSync` 当前项目目录下经 UUID
  校验的 `<session-id>.jsonl`；这不是“调用 Claude Code 自带删除功能”。
...
1. 实施前先针对项目支持的最低 Claude Code 版本和当前验证版本，确认是否存在公开、稳定的
   单会话删除 API/CLI。存在时由主进程通过结构化参数调用，不拼接 shell 字符串。
2. 不得用 `claude project purge` 代替单会话删除，不得依赖未文档化的 TUI 坐标或盲发按键。
3. 如果届时仍没有正式的单会话接口，应暂停在技术门禁处，明确记录官方能力缺口，再决定是否
   继续采用当前“严格验证后删除 JSONL”的兼容方案；不得把兼容方案描述成 Claude Code 官方 API。
...
- 顶级活动栏出现“对话”，有明确图标、可读名称、选中态与键盘焦点。
- 首次使用显示配置引导：协议、接口地址、认证方式、模型名称和可选自定义模型。
- 聊天区至少包含消息列表、多行输入、发送、流式输出、停止生成、失败重试、新建对话和清空
  当前显示；IME、`Enter`/`Shift+Enter` 与减少动态效果行为保持现有设计规范。
- 模型选择以独立对话配置为准，允许填写服务商目录之外的合法模型标识。
- 请求由主进程发起，支持取消和超时；错误区分网络、认证、限流、模型不存在、协议不兼容和
  中转非标准响应，不把密钥或完整敏感响应写入日志。
- 对话历史是否持久化、默认保留周期、导出/删除方式需要在实现前确定。未确定前不得默认长期
  保存正文；建议先提供本次运行期会话，再与“隐私与数据”设置一并设计持久化。

### 验收

- “对话”使用模型 A、“接入”使用模型 B 时，两边可以同时工作且互不改写配置。
- 修改、删除或测试独立对话 profile 不改变任何项目的 Claude Code 接入配置。
- 直连 API 和兼容中转各至少有一条可替换 HTTP 测试夹具；流式取消后不再向 UI 追加内容。
- 无凭据、错误凭据、超时、429、模型不存在和畸形流式响应都有明确、可恢复的界面状态。
...
4. **当前版本**：从打包应用实际版本读取，不手写常量；同时提供统一的“检查更新”入口和
   最近检查结果。
...
| 对话与模型     | 随任务二新增             | 独立聊天默认模型、系统提示、生成参数、历史保留         |
| 终端与项目     | 功能增长后再拆           | 默认 shell、终端行为、项目恢复与工作区偏好             |
| 通知           | 有系统通知后再拆         | 完成、错误、后台任务、声音和免打扰                     |
| 隐私与数据     | 开始持久化聊天前必须新增 | 凭据说明、会话保留、导出/清除、日志和遥测              |
...
- 左下角“全局设置”在项目、接入、插件、未来对话页以及侧栏收起状态下都可达。
- 原“接入”页不再出现高级设置入口，全部原功能在“全局设置 → 接入”可找到且只有一套实现。
- 开机启动、主题和语言重启应用后保持；版本取值与 `app.getVersion()`/安装包版本一致。
